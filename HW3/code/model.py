...
class RNN(object):
    def __init__(self,
        ...
        outputs, states = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs=self.embed_input,  dtype=tf.float32, scope="rnn")
        ...
        with tf.variable_scope('logits'):
            ...
            A = tf.transpose(tf.nn.softmax(tf.matmul(tf.nn.tanh(tf.matmul(H,expand(Ws1))), expand(Ws2))), perm=[0,2,1])# shape: [batch, param_r, 2*num_units]
            M = tf.matmul(A,H)
        ...
        self.penalized_term = tf.matmul(A, A,transpose_b=True)
        self.penalized_term=tf.norm(self.penalized_term - identity)

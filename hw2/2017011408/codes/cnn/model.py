...
class Model:
    ...
    def forward(self, is_train = True, reuse=None):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            # Your MaxPool
            out = tf.layers.max_pooling2d(out, (2,2), 2, "valid")
            # Your Conv Layer
            out = tf.layers.conv2d(out, filters = 128, kernel_size = 4)
            # Your BN Layer: use batch_normalization_layer function
            out = batch_normalization_layer(out, is_train)
            # Your Relu Layer
            out = tf.nn.relu(out)
            ...
            out = tf.layers.max_pooling2d(out, (2,2), 2, "valid")
            # Your Linear Layer
            logits = tf.layers.dense(tf.reshape(out, shape = [-1, 4608]), units = 10)
